...
var purchase_inquiry_goods_detail_grd = null;//销售合同明细列表
$(function(){
	//销售合同商品列表
	purchase_inquiry_goods_detail_grd=$('#purchase_inquiry_goods_detail_grd').datagrid({
   		fitColumns:true,
   		autoRowHeight:false,
   		border:false,
   		striped:true,
...
function specIdFormatter(value,row,index){
	return com.radius.datagrid.spec_type.formatter(value,row,index,purchase_inquiry_goods_detail_spec_type);
}
//规格格式化
function unitFormatter(value,row,index){
	return com.radius.datagrid.formatter(value,row,index,purchase_inquiry_goods_detail_spec_unit);
}
		...
		success:function(r){
			if(r&&r.total>0){
				...
				//遍历销售合同商品信息
					...
					$('#purchase_inquiry_goods_detail_grd').datagrid('acceptChanges');
				});
			}
		},error:function(r){
...
<div id="purchase_inquiry_goods_detail_grd"></div>