...
var purchase_contract_payment_detail_pay_grd = null;//采购订单列表
$(function (){
	//----------------------------初始化---------------------------------
	...
	purchase_contract_payment_detail_pay=$('#purchase_contract_payment_detail_pay').combobox({
 	...
 	$('#purchase_contract_payment_detail_deadline_time').datebox({
 	});
	...
	var purchase_contract_payment_detail_amount=$('#purchase_contract_payment_detail_amount').on('focusout',function(){//输入框失去焦点
 	...
 	$('#purchase_contract_payment_detail_add_btn').on('click',function(){
		...
		var rows=purchase_contract_payment_detail_pay_grd.datagrid('getRows');
 		var row =null;//表示需要更改的对象
 		if(rows==null||rows.length==0){
 			addPurchaseContractPayDetailDataGrid();
 			purchase_contract_payment_detail_pay_grd.datagrid('acceptChanges');
 	...
 	$('#purchase_contract_payment_detail_delete_btn').on('click',function(){
 		var rows=purchase_contract_payment_detail_pay_grd.datagrid('getSelections');
 		if(rows.length>0){
 			//弹出对话框
						...
						$.ajax({
							...
							success:function (data){//2.将dategrid的行数据进行删除操作
								if(data&&data.success){
									for(var i=0;i<rows.length;i++){
										var index=purchase_contract_payment_detail_pay_grd.datagrid('getRowIndex',rows[i]);
										purchase_contract_payment_detail_pay_grd.datagrid('deleteRow',index);
										purchase_contract_payment_detail_pay_grd.datagrid('acceptChanges');//接受改变
									}
								}
							},
							error: function(data){
...
function addPurchaseContractPayDetailDataGrid(){
	...
	//1.添加合同支付明细
	purchase_contract_payment_detail_pay_grd.datagrid('appendRow',row_data);
	//2.将剩余金额进行减少
	...
	console.info(purchase_contract_payment_detail_surplus_amount.val());
	//3.将合同支付明细提交到内存中
...
function getPurchaseContractPayDetailPayGrdRowFormatter(){
	var json = {
		...
		receiveType:$('#purchase_contract_payment_detail_pay').combobox('getText'),//收付款类型
		validityDate:$('#purchase_contract_payment_detail_deadline_time').datebox('getText'),//款项期限
		stats:'',//状态
		cashType:purchase_contract_payment_detail_currency.combobox('getText'),//币种
...
function getPurchaseContractPayDetailMemcached(row_data){
	...
	var json = {
		...
		receiveType:purchase_contract_payment_detail_pay.combobox('getValue'),//收付款类型
		validityDate:parseDate($('#purchase_contract_payment_detail_deadline_time').datebox('getValue')),//款项期限
...
<table class="table" style="width: 100%;">
	<tr>
		<th>合同总额</th>
		<td>
			<input  id="purchase_contract_payment_detail_total_amount" value="2" style="background:#eee;border:1px solid #95B8E7;" readonly="readonly"/>
			<!-- 剩余金额 -->
			<input type="hidden"  id="purchase_contract_payment_detail_surplus_amount"/>
		</td>
		<th>币种</th>
	...
	<tr>
		<th>金额</th>
		<td>
		<input style="width: 100px;border:1px solid #95B8E7;" id="purchase_contract_payment_detail_amount" value="0.00" type="text"  class="easyui-validatebox"  name="name" data-options="validType:'numberic(\'#purchase_contract_payment_detail_total_amount\')',missingMessage:'请输入金额',invalidMessage:'请输入正确的格式'"/>
		<input type="text" name="name" style="width: 20px;background:#eee;" readonly="readonly" value="元"/>
		</td>
		<th>人民币大写</th>
		<td colspan="3"><input  style="width: 400px;background:#eee;border:1px solid #95B8E7;" type="text"  id="purchase_contract_payment_detail_upper_rmb" /></td>
		<th>款项期限</th>
		<td><input id="purchase_contract_payment_detail_deadline_time"/></td>
	</tr>
	<tr>
		<th>备注</th>
