...
$(function (){
	...
	contract_sales_pay_detail_pay=$('#contract_sales_pay_detail_pay').combobox({
 	...
 	var contract_sales_pay_detail_amount=$('#contract_sales_pay_detail_amount').on('focusout',function(){//输入框失去焦点
 		if(parseFloat($(this).val())*100>parseFloat($('#contract_sales_pay_detail_surplus_amount').val())*100){
 			//$('#contract_sales_pay_detail_amount').attr('value','0');
 			document.getElementById('contract_sales_pay_detail_amount').value='0.00';
...
function addContractSalesPayDetailDataGrid(){
	//1.添加合同支付明细
	contract_sales_pay_grd.datagrid('appendRow',{
			batch:$('#contract_sales_pay_detail_batch').val(),//款项批次
			cashType:contract_sales_pay_detail_currency.combobox('getText'),//币种
			amount:$('#contract_sales_pay_detail_amount').val(),//金额
			validityDate:$('#contract_sales_pay_detail_deadline_time').datebox('getValue'),//款项期限
			receiveType:contract_sales_pay_detail_pay.combobox('getText'),//支付方式
			upperAmount:$('#contract_sales_pay_detail_upper_rmb').val(),//人民币大写
			mome:$('#contract_sale_pay_detail_memo').val()//备注
	});
	//2.将剩余金额进行减少
	...
	contract_sales_pay_detail_surplus_amount.val(surplus_amount-parseFloat($('#contract_sales_pay_detail_amount').val()));
	console.info(contract_sales_pay_detail_surplus_amount.val());
	//3.将合同支付明细提交到内存中
	$.ajax({
		...
		data:{
				...
				contractId:$('#constract_sales_sale_base_id').val(),//合同编号
				batch:$('#contract_sales_pay_detail_batch').val(),//款项批次
				amount:parseFloat($('#contract_sales_pay_detail_amount').val())*100,//金额
				...
				receiveType:contract_sales_pay_detail_pay.combobox('getValue'),//支付方式
...
<table class="table" style="width: 100%;">
	...
	<tr>
		<th>金额</th>
		<td>
		<input style="width: 100px;" id="contract_sales_pay_detail_amount" type="text" class="easyui-validatebox"  name="name" data-options="required:true,validType:'numberic(\'#contract_sales_pay_detail_total_amount\')',missingMessage:'请输入金额',invalidMessage:'请输入正确的格式'"/>
