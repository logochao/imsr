...
<!--
var customer_inquiry_open_dialog = null;
$(function(){
	$('#customer_inquiry_toolbar_new_btn').on('click',function(){
		customerInquiryInit();
	});
	//打开按钮
	$('#customer_inquiry_toolbar_open_btn').on('click',function(){
		customer_inquiry_open_dialog.dialog('open');
	});
	//挂单按钮
	$('#customer_inquiry_toolbar_pend_btn').on('click',function(){
		...
		//2.提交表单数据通过ajax
		var ajax_url = '${path}/inquiry/manager/salesInquiry/customer_inquiry_infos_add.html';
		var ajax_data=getCustomerInquiryFormAjaxDataFormatter(_status);
		submitCustomerInquiryInfo(ajax_url,ajax_data);
	});
	//提交合同按钮
	$('#customer_inquiry_toolbar_ok_btn').on('click',function(){
		var _status = $(this).attr("contract_status");
		if($(this).attr("disabled")){
			return ;
		}
	...
	//打印按钮
	$('#customer_inquiry_toolbar_print_btn').on('click',function(){
	});
	//报表按钮
	$('#customer_inquiry_toolbar_report_btn').on('click',function(){
	});
	
	customer_inquiry_open_dialog=$('#customer_inquiry_open_dialog').show().dialog({
		 title:'选择客户询价',
		 width:800,
		 height:400,
		...
		success:function(r){
			...
			if(r&&r.success){
				//1.设置对应的操作按钮
		 		...
		 		}else if ($("#customer_inquiry_toolbar_ok_btn").attr('contract_status')==r.child){//选择当前已确认的销售合同
		 			//禁用挂单按钮
		 			pend_btn.attr("disabled",true);
					pend_btn.linkbutton('disable');
					//禁用提交合同按钮
				...
				$('#customer_inquiry_base_status').combobox('setValue',r.child);//设置合同状态
			}
		},
		error:function(r){
...
function getCustomerInquiryFormAjaxDataFormatter(_status){
	var json={
			...
			customerId			:$('#customer_inquiry_base_customer_id').val(),//客户编号
			customerAb			:$('#customer_inquiry_base_customer_name').val(),//客户简称
			cashType			:customer_inquiry_base_cash_type.combobox("getValue"),//币种
			address				:$('#customer_inquiry_base_delivery_point').val(),//送货地址
			man					:$('#customer_inquiry_base_link_man').val(),//联系人
			mobile				:$('#customer_inquiry_base_link_mobile').val(),//联系人手机
			tel					:$('#customer_inquiry_base_link_tel').val(),//联系电话
			fax					:$('#customer_inquiry_base_link_fax').val(),//联系fax
			inquiryDate			:parseDate($('#customer_inquiry_base_order_time').val()),//询价日期
			latestQuotateDate	:parseDate($('#customer_inquiry_base_order_end_time').val()),//最迟报价日期
			salesman			:$('#customer_inquiry_base_sales_man').val(),//业务员
			status				:_status,//状态
			//printTimes			:$('#').val(),打印次数
			//printTime			:$('#').val(),最新打印时间
			//printRecord			:$('#').val(),//最新打印记录
			memo				:$('#customer_inquiry_memo').val()//备注
	};
	return json;
}
//-->
...
<jsp:include flush="true" page="customer_inquiry_init.jsp"></jsp:include>