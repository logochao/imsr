...
var sales_order_goods_detail_grd = null;//销售合同明细列表
$(function(){
	//销售合同商品列表
	sales_order_goods_detail_grd=$('#sales_order_goods_detail_grd').datagrid({
   		fitColumns:true,
   		autoRowHeight:false,
   		border:false,
   		striped:true,
...
function specIdFormatter(value,row,index){
	return com.radius.datagrid.spec_type.formatter(value,row,index,sales_order_goods_detail_spec_type);
}
//规格格式化
function unitFormatter(value,row,index){
	return com.radius.datagrid.formatter(value,row,index,sales_order_goods_detail_spec_unit);
}
//
function equivalentUnitFormatter(value,row,index){
	return com.radius.datagrid.formatter(value,row,index,sales_order_goods_detail_equivalent_unit);
}
		...
		success:function(r){
			if(r&&r.total>0){
				...
				//遍历销售合同商品信息
					...
					addTargetDataGridRowData($('#sales_order_goods_detail_grd'),row_data,0);
					//2.将数据添加到memcahed中
					...
					$('#sales_order_goods_detail_grd').datagrid('acceptChanges');
				});
				//3.计算
...
<div id="sales_order_goods_detail_grd"></div>